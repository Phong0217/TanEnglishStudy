...
type Classroom = { id: number; name: string; code: string; status: string; student_count: number; deliveries_count: number; course_version: { title: string; course: { title: string } }; primary_teacher?: { name: string } };
...
export default function Classes({ classes }: { classes: Classroom[] }) {
    ...
    return <AppShell><Head title="Lớp của tôi" /><PageHeader title="Lớp học của tôi" description="Các lớp đang được phân công và danh sách học sinh." />
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{classes.map(c => <Card key={c.id} className="p-5"><div className="flex justify-between gap-3"><h2 className="text-lg font-semibold">{c.name}</h2><Badge value={c.status} /></div><p className="mt-2 text-sm text-slate-500">{c.course_version.course.title} · {c.course_version.title}</p><div className="mt-5 grid grid-cols-2 gap-3"><div className="rounded-xl bg-slate-50 p-3"><p className="text-2xl font-semibold">{c.student_count}</p><p className="text-xs text-slate-500">Học sinh</p></div><div className="rounded-xl bg-slate-50 p-3"><p className="text-2xl font-semibold">{c.deliveries_count}</p><p className="text-xs text-slate-500">Bài học đã giao</p></div></div></Card>)}{!classes.length && <div className="md:col-span-2"><EmptyState title="Chưa được phân công lớp" description="Quản trị viên sẽ phân công lớp giảng dạy cho bạn." /></div>}</div>
    </AppShell>;
}

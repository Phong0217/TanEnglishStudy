...
class ClassroomController extends Controller
{
    ...
    public function store(StoreClassroomRequest $request, AuditLogger $audit): RedirectResponse
    {
        ...
        $classroom = DB::transaction(function () use ($data, $request) {
            ...
            }$classroom = Classroom::create([...collect($data)->except('primary_teacher_id')->all(), 'center_id' => $request->user()->center_id, 'course_version_id' => null, 'primary_teacher_id' => $teacher?->id, 'status' => 'ACTIVE', 'created_by' => $request->user()->id]);
            if ($teacher) {
